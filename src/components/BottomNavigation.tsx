...
const BottomNavigation: React.FC<BottomNavigationProps> = ({ activeTab: propActiveTab }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50">
      ...
      <div className="bg-white border-t border-gray-100 rounded-t-xl shadow-lg">
        <div className="flex justify-between items-center px-2 py-1 mx-auto">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className="flex flex-col items-center justify-center pt-2 pb-1 px-3 transition-all"
              >
                ...
                {active && (
                  <div className="w-1.5 h-1.5 bg-blue-600 rounded-full absolute -bottom-0.5" />
                )}
              </Link>
            );
          })}

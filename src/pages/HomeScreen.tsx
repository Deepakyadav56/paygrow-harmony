...
const HomeScreen: React.FC = () => {
  return (
    <div className="pb-20"> {/* Add padding at bottom for the nav */}
      {/* Header */}
      <div className="bg-gradient-to-r from-fountain-blue-500 to-fountain-blue-700 text-white pt-12 pb-6 px-4 rounded-b-3xl shadow-lg">
        ...
        <div className="grid grid-cols-4 gap-2">
          {quickActions.map((action, index) => (
            <Link key={index} to={action.path} className="text-center">
              <div className="w-14 h-14 rounded-full bg-fountain-blue-50 flex items-center justify-center mx-auto mb-1 hover:bg-fountain-blue-100 transition-colors">
                <span className="text-fountain-blue-600">{action.icon}</span>
              </div>
      ...
      {/* Investment Summary */}
      <Card className="mx-4 my-6 p-4 bg-gradient-to-r from-fountain-blue-600 to-fountain-blue-700 text-white rounded-xl">
      ...
      <div className="p-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to="/transactions" className="text-sm text-fountain-blue-600">See All</Link>
        </div>
        
        <div className="space-y-3">
          {recentTransactions.map((transaction) => (
            <Card key={transaction.id} className="p-3 flex justify-between items-center hover:shadow-md transition-shadow">
              <div className="flex items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${
                  ...
                  {transaction.type === 'investment' && <BarChartIcon className="w-5 h-5 text-fountain-blue-500" />}
                </div>
                <div>
